...
{
    private readonly static Dictionary<Type, PropertyInfo[]> _propertyCache = new();
    private readonly static Dictionary<Type, MethodInfo[]> _methodCache = new();

    public SimpleEntityToDtoMapper()
    {
        LocaleConfiguration = new DefaultLocaleConfiguration();
    }
    ...
    protected virtual void Map(IAggregateEntity entity, ref object dto, bool mapNestedDtos)
    {
        ...
        else
        {
            ...
            foreach (var dtoProperty in dtoProperties)
            {
                ...
                if (isKeyProperty)
                {
                    dtoProperty.SetValue(dto, entity.Key);
                }
                else
                {
                    ...
                    var entityProperty = entityProperties.FirstOrDefault(x => x.Name == dtoProperty.Name);
        ...
        )
    {
        ...
        if (dtoPropertyType == entityPropertyType)
        {
            ...
            var configuration = LocaleConfiguration;
    ...
    private static bool IsNestedPropertyType(Type propertyType)
    {
