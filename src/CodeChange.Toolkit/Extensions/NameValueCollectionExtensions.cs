...
public static class NameValueCollectionExtensions
{
    ...
    public static Dictionary<string, string?> ToDictionary(this NameValueCollection col)
    {
        ...
        foreach (var key in col.AllKeys.Where(x => x != null))
        {
    ...
    /// <returns>A dictionary representing the name value collection</returns>
    public static Dictionary<TKey, TValue?> ToDictionary<TKey, TValue>(this NameValueCollection collection)
        where TKey : notnull
    {
        ...
        foreach (string name in collection)
        {
            if (name != null)
            {
                var key = (TKey?)keyConverter.ConvertFromString(name!) ?? default;
                var value = (TValue?)valueConverter.ConvertFromString(collection[name!] ?? String.Empty);
