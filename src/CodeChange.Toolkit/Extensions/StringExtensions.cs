...
public static class StringExtensions
{
    ...
    /// </summary>
    private static readonly Regex _apostropheRegex = new(@"'|’", RegexOptions.IgnoreCase | RegexOptions.Compiled);

    /// <summary>
    /// Defines a compiled RegEx object for matching bad URL characters
    /// </summary>
    private static readonly Regex _urlBadCharRegex = new(@"[^a-z0-9]", RegexOptions.IgnoreCase | RegexOptions.Compiled);

    /// <summary>
    /// Defines a compiled RegEx object for matching multiple spaces
    /// </summary>
    private static readonly Regex _multiSpaceRegex = new(@" +", RegexOptions.IgnoreCase | RegexOptions.Compiled);

    /// <summary>
    /// Defines a compiled RegEx object for matching HTML tags
    /// </summary>
    private static readonly Regex _htmlRegex = new(@"<.*?>", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    ...
    public static string Truncate(this string input, int length, string appendWith = "...")
    {
        ...
        else
        {
            ...
            {
                return $"{input[..length]}{appendWith}";
            }
            else
            {
                return input;
    ...
    {
        return Truncate(input, length, String.Empty);
    }
